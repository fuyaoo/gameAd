    ...
    private static BufferedImage genBarcode(String content, int width, int height, String srcImagePath)throws WriterException,IOException {
        ...
        for (int y = 0; y < matrix.getHeight(); y++) {
            for (int x = 0; x < matrix.getWidth(); x++) {
                // 左上角颜色,根据自己需要调整颜色范围和颜色 #FFC500 255,197, 0   231, 144, 56
                if (x > 0 && x < 170 && y > 0 && y < 170) {
                    Color color = new Color(0,118,164);
...
//                    Color color = new Color(0, 0, 255);
                    ...
                    int colorInt = color.getRGB();
                    // 此处可以修改二维码的颜色，可以分别制定二维码和背景的颜色； 23518525 16777215
    ...
    public static void  main(String[] args) throws UnsupportedEncodingException {
        ...
        String content = "http://wx.goldmister.com/#/login?phoneNumber="+1234587+"正邀请你注册黄金先生";
        String srcImagePath = "D:/broker/broker_user.png";
        String destImagePath = "D:/broker/demo/test.png";
        // 依次为内容(不支持中文),宽,长,中间图标路径,储存路径 
        QRcodeUtil.encode(content, 512, 512, srcImagePath, destImagePath);
    }
}
