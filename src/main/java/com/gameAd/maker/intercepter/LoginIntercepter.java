...
public class LoginIntercepter implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        if (TextUtils.isEmpty(token)) {
            ...
            writeToResponse(response, JSON.toJSONString(new ResultObj(ResultStatus.TOKEN_IS_EMPTY)));
            return false;
        }
        ...
        if (MapUtils.isEmpty(map)) {
            ...
            writeToResponse(response, JSON.toJSONString(new ResultObj(ResultStatus.PARAMETERS_EXCEPTION)));
            return false;
        }
        //校验时间是否超过24小时
        ...
        try {
            ...
            LOGGER.error("日期比对失败");
            writeToResponse(response, JSON.toJSONString(new ResultObj(ResultStatus.LOGIN_TIMEOUT)));
            return false;
        }
        if(! flag){
